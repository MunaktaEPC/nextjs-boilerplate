...
export default function RoboCupPortal() {
  ...
  async function handleBlogSubmit() {
    ...
    await fetchData(); // 先にデータを更新
    setView('blog_list'); // その後に一覧へ
    setLoading(false);
  }
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f8f9fa', color: '#333' }}>
      ...
      <main style={{ maxWidth: '1100px', margin: '0 auto', padding: '30px 20px' }}>
        ...
        {loading && <div style={{ textAlign: 'center', padding: '20px', color: '#5a3d8a', fontWeight: 'bold' }}>読み込み中...</div>}
        ...
        {view === 'bbs_read' && activeThread && (
          <div>
            ...
            <div style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '12px', border: '1px solid #ddd' }}>
              ...
              <button onClick={() => handleReplySubmit(activeThread.id, setReplyContent, replyContent)} style={{ backgroundColor: '#5a3d8a', color: '#fff', border: 'none', padding: '10px 40px', borderRadius: '8px', cursor: 'pointer' }}>送信</button>
            </div>
          </div>
        )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {mainThreads.map(t => (
                <article key={t.id} onClick={() => { setActiveThread(t); setView('bbs_read'); }} style={{ backgroundColor: '#fff', padding: '15px 20px', borderRadius: '10px', border: '1px solid #eee', cursor: 'pointer', display: 'flex', justifyContent: 'space-between' }}>
                  <div><h4 style={{ margin: '0 0 5px 0', color: '#d32f2f' }}>{t.title}</h4><div style={{ fontSize: '12px', color: '#888' }}>{t.author_name} · {new Date(t.created_at).toLocaleString()}</div></div>
                </article>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '30px' }}>
            <aside style={{ width: '200px', flexShrink: 0 }}>
              ...
              <ul style={{ listStyle: 'none', padding: 0, marginTop: '15px' }}>
                <li onClick={() => setSelectedGenre(null)} style={{ cursor: 'pointer', color: !selectedGenre ? '#d32f2f' : '#555', marginBottom: '10px' }}>すべて表示</li>
                {existingGenres.map(g => (
                  <li key={g} onClick={() => setSelectedGenre(g)} style={{ cursor: 'pointer', color: selectedGenre === g ? '#d32f2f' : '#555', marginBottom: '10px' }}>{g}</li>
                ))}
              </ul>
            </aside>
        ...
        {view === 'blog_write' && (
          <div style={{ maxWidth: '800px', margin: '0 auto', backgroundColor: '#fff', padding: '40px', borderRadius: '15px', border: '1px solid #ddd' }}>
            ...
            <div style={{ marginBottom: '25px', border: '2px dashed #ddd', height: '180px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '10px', position: 'relative' }}>
