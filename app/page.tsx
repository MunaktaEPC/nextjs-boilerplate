...
export default function MunakataPortal() {
  ...
  const renderContent = (text: string) => {
    ...
    return parts.map((part, index) => {
      ...
      if (fileMatch) return (
        <a key={index} href={fileMatch[2]} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-block', backgroundColor: '#f0f0f0', padding: '10px 15px', borderRadius: '8px', textDecoration: 'none', color: '#5a3d8a', fontWeight: 'bold', margin: '8px 0', border: '1px solid #ddd' }}>
          📎 {fileMatch[1]}
        </a>
      );
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f8f9fa', color: '#333', fontFamily: 'sans-serif' }}>
      ...
      {/* ヘッダー */}
      <header style={{ backgroundColor: '#fff', borderBottom: '4px solid #5a3d8a', padding: '12px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px', cursor: 'pointer' }} onClick={() => setView('home')}>
          ...
          <h1 style={{ margin: 0, color: '#5a3d8a', fontSize: '22px', fontWeight: '900' }}>MunakataEPC_PORTAL</h1>
        </div>
      ...
      {/* メインナビ */}
      <nav style={{ padding: '12px 40px', display: 'flex', gap: '12px', borderBottom: '1px solid #eee', backgroundColor: '#fff' }}>
        <button onClick={() => setView('home')} style={{ padding: '10px 25px', borderRadius: '25px', border: 'none', backgroundColor: view === 'home' ? '#5a3d8a' : '#f0f0f0', color: view === 'home' ? '#fff' : '#555', cursor: 'pointer', fontWeight: 'bold' }}>🏠 ホーム</button>
        <button onClick={() => setView('bbs')} style={{ padding: '10px 25px', borderRadius: '25px', border: 'none', backgroundColor: view.startsWith('bbs') ? '#5a3d8a' : '#f0f0f0', color: view.startsWith('bbs') ? '#fff' : '#555', cursor: 'pointer', fontWeight: 'bold' }}>💬 掲示板</button>
        <button onClick={() => setView('blog_list')} style={{ padding: '10px 25px', borderRadius: '25px', border: 'none', backgroundColor: view.startsWith('blog') ? '#5a3d8a' : '#f0f0f0', color: view.startsWith('blog') ? '#fff' : '#555', cursor: 'pointer', fontWeight: 'bold' }}>🖋️ ブログ</button>
      </nav>
      ...
      <main style={{ maxWidth: '1100px', margin: '0 auto', padding: '30px 20px' }}>
        ...
        {view === 'home' && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '25px' }}>
            <div onClick={() => setView('bbs')} style={{ backgroundColor: '#fff', borderRadius: '20px', padding: '40px', cursor: 'pointer', textAlign: 'center', border: '1px solid #ddd', transition: '0.2s' }}>
              <div style={{ fontSize: '50px', marginBottom: '20px' }}>💬</div>
              <h3 style={{ margin: 0, fontSize: '22px' }}>交流掲示板</h3>
              <p style={{ color: '#888' }}>部員同士の質疑応答・雑談</p>
            </div>
            <div onClick={() => setView('blog_list')} style={{ backgroundColor: '#fff', borderRadius: '20px', padding: '40px', cursor: 'pointer', textAlign: 'center', border: '1px solid #ddd', transition: '0.2s' }}>
              <div style={{ fontSize: '50px', marginBottom: '20px' }}>🖋️</div>
              <h3 style={{ margin: 0, fontSize: '22px' }}>活動ブログ</h3>
              <p style={{ color: '#888' }}>技術情報の共有・大会記録</p>
            </div>
          </div>
        )}
        ...
        {view === 'bbs' && (
          <div>
            <section style={{ backgroundColor: '#f3eef7', padding: '25px', borderRadius: '15px', marginBottom: '30px', border: '1px solid #e0d5ed' }}>
              ...
              <textarea value={content} onChange={(e) => setContent(e.target.value)} placeholder="内容を入力してください..." style={{ width: '100%', height: '100px', padding: '12px', borderRadius: '8px', border: '1px solid #ccc' }} />
              <button onClick={handleBbsSubmit} style={{ marginTop: '10px', backgroundColor: '#5a3d8a', color: '#fff', padding: '12px 35px', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' }}>投稿する</button>
            </section>
            {mainThreads.map(t => (
              <article key={t.id} onClick={() => { setActiveThread(t); setView('bbs_read'); }} style={{ backgroundColor: '#fff', border: '1px solid #eee', borderRadius: '15px', padding: '20px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '20px', marginBottom: '15px' }}>
                <img src={t.author_avatar || 'https://via.placeholder.com/45'} style={{ width: '45px', height: '45px', borderRadius: '50%', objectFit: 'cover' }} />
                <div style={{ flexGrow: 1 }}>
                  <h4 style={{ margin: '0 0 5px 0', color: '#b91c1c', fontSize: '18px' }}>{t.title}</h4>
                  <div style={{ fontSize: '13px', color: '#888' }}>{t.author_name} · {new Date(t.created_at).toLocaleString()}</div>
                </div>
                {isAdmin && <button onClick={(e) => handleDeletePost(t.id, e)} style={{ border: 'none', background: 'none', color: '#ccc' }}>🗑️</button>}
              </article>
            ))}
          </div>
        )}
        ...
        {view === 'bbs_read' && activeThread && (
          <div>
            ...
            <div style={{ backgroundColor: '#fff', border: '1px solid #ddd', borderRadius: '15px', padding: '30px', marginBottom: '30px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '20px' }}>
                ...
                <div>
                  <h2 style={{ margin: 0, color: '#b91c1c' }}>{activeThread.title}</h2>
            ...
            <div style={{ marginBottom: '30px' }}>
              {getReplies(activeThread.id).map(r => (
                <div key={r.id} style={{ display: 'flex', gap: '15px', marginBottom: '15px' }}>
                  <img src={r.author_avatar || 'https://via.placeholder.com/40'} style={{ width: '40px', height: '40px', borderRadius: '50%' }} />
                  <div style={{ flexGrow: 1, backgroundColor: '#fff', padding: '15px', borderRadius: '12px', border: '1px solid #eee', position: 'relative' }}>
                    <div style={{ fontWeight: 'bold', fontSize: '13px', marginBottom: '5px' }}>{r.author_name}</div>
            ...
            <div style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '15px', border: '1px solid #ddd' }}>
              <textarea value={replyContent} onChange={(e) => setReplyContent(e.target.value)} placeholder="返信する..." style={{ width: '100%', height: '80px', padding: '12px', borderRadius: '8px', border: '1px solid #eee' }} />
              <button onClick={() => handleReplySubmit(activeThread.id)} style={{ marginTop: '10px', backgroundColor: '#5a3d8a', color: '#fff', padding: '10px 30px', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>送信</button>
            </div>
          </div>
        )}
          ...
          <div style={{ display: 'flex', gap: '30px' }}>
            <aside style={{ width: '220px', flexShrink: 0 }}>
              ...
              <ul style={{ listStyle: 'none', padding: 0, marginTop: '15px' }}>
                <li onClick={() => setSelectedGenre(null)} style={{ cursor: 'pointer', color: !selectedGenre ? '#b91c1c' : '#555', padding: '8px 0', fontWeight: !selectedGenre ? 'bold' : 'normal' }}>すべて表示</li>
                {existingGenres.map(g => (
                  <li key={g} onClick={() => setSelectedGenre(g)} style={{ cursor: 'pointer', color: selectedGenre === g ? '#b91c1c' : '#555', padding: '8px 0', fontWeight: selectedGenre === g ? 'bold' : 'normal' }}>{g}</li>
                ))}
              </ul>
            </aside>
            <div style={{ flexGrow: 1 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px' }}>
                <h2 style={{ margin: 0 }}>活動ブログ</h2>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '20px' }}>
                {blogArticles.filter(a => !selectedGenre || a.genre === selectedGenre).map(a => (
                  <article key={a.id} onClick={() => { setActiveArticle(a); setView('blog_read'); }} style={{ backgroundColor: '#fff', border: '1px solid #eee', borderRadius: '15px', overflow: 'hidden', cursor: 'pointer' }}>
                    ...
                    <div style={{ padding: '18px' }}>
                      <span style={{ fontSize: '11px', backgroundColor: '#5a3d8a', color: '#fff', padding: '3px 10px', borderRadius: '12px' }}>{a.genre}</span>
                      <h4 style={{ margin: '12px 0', fontSize: '17px', lineHeight: '1.4' }}>{a.title}</h4>
        ...
        {view === 'blog_read' && activeArticle && (
          <article style={{ maxWidth: '850px', margin: '0 auto', backgroundColor: '#fff', borderRadius: '20px', border: '1px solid #eee', overflow: 'hidden' }}>
            {activeArticle.image_url && <img src={activeArticle.image_url} style={{ width: '100%', maxHeight: '400px', objectFit: 'cover' }} />}
            <div style={{ padding: '40px' }}>
              ...
              <div style={{ color: '#5a3d8a', fontWeight: 'bold', fontSize: '14px', marginBottom: '10px' }}>📁 {activeArticle.genre}</div>
              <h1 style={{ fontSize: '32px', margin: '0 0 25px 0', color: '#222' }}>{activeArticle.title}</h1>
              <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '40px', paddingBottom: '20px', borderBottom: '1px solid #eee' }}>
                ...
                <button onClick={() => handleLike(activeArticle)} style={{ backgroundColor: likedPosts.includes(activeArticle.id) ? '#ff4b5c' : '#fff', border: '1px solid #ff4b5c', color: likedPosts.includes(activeArticle.id) ? '#fff' : '#ff4b5c', padding: '8px 20px', borderRadius: '20px', cursor: 'pointer', fontWeight: 'bold' }}>❤️ {activeArticle.likes || 0}</button>
                {isAdmin && <button onClick={() => handleDeletePost(activeArticle.id)} style={{ color: '#d32f2f', background: 'none', border: '1px solid #d32f2f', padding: '8px 15px', borderRadius: '20px', cursor: 'pointer' }}>🗑️</button>}
              </div>
              <div style={{ fontSize: '18px', lineHeight: '2', color: '#444' }}>{renderContent(activeArticle.content)}</div>
            </div>
          </article>
        )}
        ...
        {view === 'blog_write' && (
          <div style={{ maxWidth: '850px', margin: '0 auto', backgroundColor: '#fff', padding: '40px', borderRadius: '20px', border: '1px solid #ddd' }}>
            ...
            <div style={{ margin: '25px 0', height: '250px', border: '2px dashed #ccc', borderRadius: '15px', display: 'flex', justifyContent: 'center', alignItems: 'center', position: 'relative', overflow: 'hidden' }}>
              {coverPreview ? <img src={coverPreview} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ color: '#888' }}>📷 タイトル画像を追加</span>}
            ...
            <div style={{ marginBottom: '20px' }}>
              <label style={{ display: 'block', fontWeight: 'bold', marginBottom: '8px' }}>ジャンル</label>
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <input type="text" placeholder="新規ジャンル名" onChange={(e) => setGenre(e.target.value)} style={{ width: '180px', padding: '12px', borderRadius: '8px', border: '1px solid #ccc' }} />
              </div>
            </div>
            ...
            <textarea value={content} onChange={(e) => setContent(e.target.value)} placeholder="本文を入力してください...（ファイルを添付するには[名前](URL)の形式で記述します）" style={{ width: '100%', minHeight: '400px', border: 'none', outline: 'none', fontSize: '18px', lineHeight: '1.6' }} />
          ...
          <section style={{ maxWidth: '500px', margin: '0 auto', backgroundColor: '#fff', padding: '40px', borderRadius: '20px', border: '1px solid #ddd' }}>
            ...
            <div style={{ marginBottom: '25px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                <img src={avatarPreview || 'https://via.placeholder.com/60'} style={{ width: '60px', height: '60px', borderRadius: '50%', objectFit: 'cover' }} />
